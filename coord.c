...
void coord_init_after_last_item (coord_t *coord, znode *node)
{
	...
	coord_init_values (coord, node, (is_empty ? 0 : node_num_items (node) - 1), 0, (is_empty ? EMPTY_NODE : AFTER_ITEM));
...
/* Audited by: green(2002.06.15) */
unsigned coord_num_items (const coord_t * coord)
{
...
/* Audited by: green(2002.06.15) */
unsigned coord_last_unit_pos (const coord_t * coord)
{
...
/* Audited by: green(2002.06.15) */
unsigned coord_item_pos (const coord_t *coord)
{
...
/* Audited by: green(2002.06.15) */
int coord_is_rightmost_unit (const coord_t *coord)
{
...
/* Audited by: green(2002.06.15) */
int coord_is_rightmost_item (const coord_t *coord)
{
...
/* Audited by: green(2002.06.15) */
int coord_eq (const coord_t *c1, const coord_t *c2)
{
...
/* Audited by: green(2002.06.15) */
int coord_is_delimiting (coord_t *coord, sideof dir)
{
...
/* Audited by: green(2002.06.15) */
int coord_set_to_unit (coord_t *coord)
{
...
/* Hack. */
/* Audited by: green(2002.06.15) */
void coord_to_tcoord (coord_t *tcoord, const coord_t *ncoord)
{
...
void tcoord_to_ncoord (coord_t *ncoord, const coord_t *tcoord)
{
	...
	if (tcoord->node == NULL) {
		coord_init_invalid (ncoord, NULL);
		return;
	}
	...
	assert ("jmacd-5113", ncoord->unit_pos <= coord_last_unit_pos (ncoord));
}

/*item_plugin* item_plugin_by_ncoord (const coord_t *ncoord)
{
	coord_t tcoord;
	coord_to_tcoord (& tcoord, ncoord);
		...
		znode * left)
{
	int ret;
/*	coord_t tfrom, tto;
	coord_to_tcoord (&tfrom, from);
	coord_to_tcoord (&tto, to);*/
