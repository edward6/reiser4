...
write_hole(struct inode *inode, reiser4_cluster_t * clust, loff_t file_off, loff_t to_file)
{
	...
	if (!clust->delta) {
		...
		result = update_inode_and_sd_if_necessary(inode, pg_to_off(clust->index) + clust->off + clust->count, 1, 1, 1);
		if (result)
			return result;
...
write_cryptcompress_flow(struct file * file , struct inode * inode, const char *buf, size_t count, loff_t pos)
{
	...
	do {
		...
		result = update_inode_and_sd_if_necessary(inode,
							  pg_to_off(clust.index) + clust.off + clust.count /* new_size */,
							  (pg_to_off(clust.index) + clust.off + clust.count > inode->i_size) ? 1 : 0,
							  1, 
							  1/* update stat data */);
		if (result)
			goto exit1;
