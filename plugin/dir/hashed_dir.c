								 ...
								 * particular */ )
{
	...
	trace_on(TRACE_RESERVE, "hashed_init grabs %llu blocks.", reserve);
...
hashed_done(struct inode *object /* object being deleted */ )
{
	...
	if (!inode_get_flag(object, REISER4_NO_SD)) {
		...
		trace_on(TRACE_RESERVE, "hashed_done grabs %llu blocks.", reserve);
	      ...
	      struct dentry *new_name /* new name */ )
{
	...
	old_coord = &old_fsdata->entry_coord;
	new_coord = &new_fsdata->entry_coord;
	...
	trace_on(TRACE_RESERVE, "hashed rename grabs %llu blocks.", reserve);
