...
int extent_can_contain_key(const coord_t * coord,
			   const reiser4_key * key, const reiser4_item_data *);
...
int extent_utmost_child_real_block(const coord_t * coord,
				   sideof side, reiser4_block_nr * block);
reiser4_key *extent_max_key(const coord_t * coord, reiser4_key * key);
 ...
 */
int extent_write(struct inode *, struct sealed_coord *, flow_t *);
int extent_read(struct inode *, struct sealed_coord *, flow_t *);
...
int allocate_extent_item_in_place(coord_t *, lock_handle *,
				  flush_position * pos);
int allocate_and_copy_extent(znode * left, coord_t * right,
			     flush_position * pos, reiser4_key * stop_key);
