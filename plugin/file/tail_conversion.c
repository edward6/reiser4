...
static int cut_formatting_items(struct inode *inode, loff_t offset, int count)
{
	 ...
	 */
	build_body_key_unix_file(inode, offset, &from, WRITE_OP);
	/*
...
static int reserve_tail2extent_iteration(struct inode *inode, loff_t offset)
{
	 ...
	 */
	grab_space_enable();
	ret = reiser4_grab_space(TAIL2EXTENT_PAGE_NUM, BA_CAN_COMMIT, subv_d);
	if (ret)
		return ret;
	/*
...
static int find_start(struct inode *inode, reiser4_plugin_id id, __u64 *offset)
{
	...
	result = 0;
	found = 0;
	build_body_key_unix_file(inode, *offset, &key, READ_OP);
	do {
		init_lh(&lh);
...
int tail2extent(struct unix_file_info *uf_info)
{
	 ...
	 */
	build_body_key_unix_file(inode, offset, &key, WRITE_OP);
