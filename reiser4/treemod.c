...
static int add_child_ptr( znode *parent, znode *child )
{
	...
	assert( "nikita-1115", parent -> level == child -> level + 1 );
 ...
 */
static int do_kill_root( reiser4_tree *tree /* tree from which root is being
					     * removed */, 
			 znode *old_root /* root node that is being removed */, 
			 znode *new_root /* new root---sole child of
					  * @old_root */, 
			 const reiser4_disk_addr *new_root_blk /* disk address
								* of
								* @new_root */ )
{
	znode *fake;
	...
	assert( "nikita-1199", new_root -> level + 1 == old_root -> level );
	...
	if( !IS_ERR( fake ) ) {
		...
		if( result == 0 ) {
			...
			_reiser4_sibling_list_insert( new_root, NULL );
			spin_unlock_tree( tree );
 ...
 * kill_tree_root() collects all necessary arguments and calls do_kill_root()
...
{
	int           result;
	tree_coord   down_link;
	znode        *new_root;

	assert( "nikita-1194", old_root != NULL );
	assert( "nikita-1196", znode_is_root( old_root ) );
	assert( "nikita-1200", num_items( old_root ) == 1 );
	assert( "nikita-1401", znode_is_write_locked( old_root ) );

	reiser4_init_coord( &down_link );
	down_link.node = old_root;
	coord_first_unit( &down_link );

	spin_lock_dk( current_tree );
	new_root = get_child_znode( &down_link, 1 );
	spin_unlock_dk( current_tree );
	if( !IS_ERR( new_root ) ) {
		result = do_kill_root( current_tree, old_root, new_root, 
				       znode_get_block( new_root ) );
		zput( new_root );
	} else
