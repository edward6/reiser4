 ...
 * See fs/reiser4/do_carry.c for details.
 */
...
typedef struct carry_node {
	...
	/** base node from which real_node is calculated. See
	    fs/reiser4/do_carry.c:lock_carry_node(). */
	znode              *node;
 ...
 *
 * Operations that do_carry() can handle. This list is supposed to be
 * expanded.
 *
 * Each carry operation (cop) is handled by appropriate function defined
 * in fs/reiser4/do_carry.c. For example COP_INSERT is handled by
 * fs/reiser4/do_carry.c:do_carry_insert() etc. These functions in turn
...
int do_carry( carry_level *doing, carry_level *done );

carry_node *reiser4_add_carry( carry_level *level, pool_ordering order,
			       carry_node  *reference );
carry_op *reiser4_add_op( carry_level *level, pool_ordering order,
			  carry_op *reference );

extern carry_node *find_begetting_brother( carry_node *node, carry_level *kin );

extern void reiser4_init_carry_pool( carry_pool *pool );
extern void reiser4_done_carry_pool( carry_pool *pool );

extern void reiser4_init_carry_level( carry_level *level, carry_pool *pool );

extern carry_op *reiser4_post_carry( carry_level *level, carry_opcode op,
				     znode *node, int apply_to_parent );
extern carry_node *reiser4_add_to_carry( znode *node, carry_level *queue );

extern int reiser4_carry_op_num( const carry_level *level );
extern int reiser4_carry_node_num( const carry_level *level );
