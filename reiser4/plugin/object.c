 ...
 */
static ssize_t common_io( struct file *file, char *buf, size_t size, 
			 loff_t *off, rw_op op )
{
	struct inode        *inode;
	ssize_t              result;
	...
	assert( "nikita-415", file != NULL );

	inode  = file -> f_dentry -> d_inode;
	assert( "nikita-416", inode != NULL );
	assert( "nikita-418", buf != NULL );
	assert( "nikita-420", off != NULL );
	assert( "nikita-421", ( op == READ_OP ) || ( op == WRITE_OP ) );

	/* file doesn't have name yet, wait a second */
	if( ( *reiser4_inode_flags( inode ) & REISER4_IMMUTABLE ) &&
	    ( op == WRITE_OP ) )
		return -EAGAIN;
	if( perm_chk( inode, rw, file, buf, size, off, op ) )
		return -EPERM;

	plugin = reiser4_get_file_plugin( inode );
	assert( "nikita-417", plugin != NULL );
	/*
	 * construct flow from buffer, size, and offset
	 */
	result = plugin -> build_flow( file, buf, size, off, op, &aflow );
	if( result )
		return result;

	/* 
	 * reserve space in transaction. If this is outermost
	 * transaction, initialise "trans", otherwise, use already
	 * existing transaction handle. This assumes some effective way
	 * to find active transaction handle for current process.  Best
	 * is to have dedicated pointer in task_struct, otherwise we can
	 * use hash-table or something similar
	 *
	 * In more details: on entry into reiser4 specific part of
	 * system call, transaction context is created as part of
	 * reiser4_context. When new transaction started, transaction
	 * manager is able to find its context somehow (by following
	 * pointer in current thread). But it is possible, that
	 * transaction manager will be asked to start transaction in the
	 * middle of active transaction (nested or, recursive
	 * transactions). For example, writing into some file would
	 * require update to the quota file. In such situation,
	 * transaction manager should be able to attach itself to the
	 * outermost active transaction.
	 *
	 */
	reserved = plugin -> estimate.rw( file, buf, size, off, op );
	result = txn_reserve( reserved );
	if( result == 0 ) {
		rw_f_type fun;
