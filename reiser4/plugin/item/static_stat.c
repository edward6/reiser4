...
int sd_save( struct inode *inode, char **area )
{
	...
	result = 0;
	emask = reiser4_get_object_state( inode ) -> extmask;
...
static int unix_sd_present( struct inode *inode, char **area, int *len )
{
	...
	if( *len >= ( int ) sizeof( reiser4_unix_stat ) ) {
		...
		reiser4_get_object_state( inode ) -> bytes = 
			d64tocpu( &sd -> bytes );
...
{
	inode -> i_uid = reiser4_get_super_data( inode -> i_sb ) -> default_uid;
	inode -> i_gid = reiser4_get_super_data( inode -> i_sb ) -> default_gid;
	inode -> i_atime = inode -> i_mtime = inode -> i_ctime = CURRENT_TIME;
	reiser4_get_object_state( inode ) -> bytes = inode -> i_size;
	/* mark inode as lightweight, so that caller (reiser4_lookup)
	   will complete initialisation by copying [ug]id from a
	   parent.*/
	reiser4_get_object_state( inode ) -> flags |= REISER4_LIGHT_WEIGHT_INODE;
	return 0;
}
...
static int unix_sd_save( struct inode *inode, char **area )
{
	...
	cputod64( reiser4_get_object_state( inode ) -> bytes, &sd -> bytes );
	*area += sizeof *sd;
	return 0;
}
...
static int len_for( reiser4_plugin *plugin, struct inode *inode, int len )
{
	...
	if( reiser4_get_object_state( inode ) -> plugin_mask & 
	    ( 1 << ( plugin -> h.type_id ) ) ) {
