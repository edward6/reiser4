...
/* generic_file_write starts with getting locked page,
   ...
   generic_file_read uses readpage. Generic_readpage accepts locked page,
   create empty buffers if necessary, then for every buffer
 */
struct page * reiser4_get_page (struct inode * inode, unsigned long long offset)
{
        unsigned long idx;
	struct page * page;
        ...
        page = grab_cache_page (inode->i_mapping, idx);
        if (IS_ERR (page))
                return page;
        
        /* have block_prepare_write to slice page into buffers and exit */
	if (!page->buffers)
...
/* return 0 if not all page buffers we will write a flow to are mapped, number
   of bytes of a flow which have to be written to this page otherwise */
int can_write (struct page * page, flow * f UNUSED_ARG)
{
...
static int data_exchange (struct page * page, flow * f, unsigned count,
			  int is_read)
{
        char * kaddr;
	unsigned offset;
	...
	if (is_read &&
	    get_key_offset (&f->key) + count >
	    (unsigned long long)page->mapping->host->i_size)
		/* do not read more than upto file size */
		count = page->mapping->host->i_size - get_key_offset (&f->key);

	offset = (get_key_offset (&f->key) & (PAGE_SIZE - 1));
	if (count > PAGE_SIZE - offset)
		count = PAGE_SIZE - offset;

        kaddr = kmap (page);
	if (is_read)
		result = __copy_to_user (f->data, kaddr + offset, count);
	else
		result = __copy_from_user (kaddr + offset, f->data, count);
	kunmap (page);
	if (result)
                return -EFAULT;
...
int reiser4_copy_to_page (struct page * page, flow * f, unsigned count)
{
	return data_exchange (page, f, count, 0 /* write */);
}


int reiser4_copy_from_page (struct page * page, flow * f, unsigned count)
{
	return data_exchange (page, f, count, 1 /* read */);
}


void reiser4_put_page (struct page * page)
{
	UnlockPage (page);
	page_cache_release (page);
}
