...
#   simple and clear 3-queue algorithm in do_carry() archives two
...
#  below: do_carry.c contains a lot more than carry implementation.
...
well as the carry code.  IFF we do this, do_carry.c maybe makes sense.

I would feel more supportive of the use of the do_carry code if a way to
 ...
 *         handle error
 *     } else {
 *         // fill in remaining fields in @op, according to do_carry.h:carry_op
 *         result = do_carry( &lowest_level, NULL );
 *     }
 ...
 */
static int do_level( carry_level *doing, carry_level *todo );
 ...
 * more operations to be carried upward etc. do_carry() takes care about
 * locking and pinning znodes while operating on them.
 *
 * For usage, see comment at the top of fs/reiser4/do_carry.c
 *
 **/
int do_carry( carry_level *doing /*
				  * set of carry operations to be performed
				  */, 
	      carry_level *done  /*
				  * set of nodes, already performed at the *
				  ...
				  */ )
{
	...
	while( ( result == 0 ) && ( reiser4_carry_op_num( doing ) > 0 ) ) {
		 ...
		 *
		 * (2) do_level() decides that more nodes have to
		 * be involved.
		 *
 ...
 *
 **/
static int do_level( carry_level *doing /* queue of carry operations to do on
					 * this level */, 
		     carry_level *todo  /* queue where new carry operations to
					 * be performed on the parent level
					 * are accumulated during @doing
					 * processing. */ )
{
	int result;
 ...
 * call do_carry( level, ... ). This function takes write lock on @node. Carry
...
carry_node *find_begetting_brother( carry_node *node, carry_level *kin )
{
	...
	assert( "nikita-1616", get_lock_counters() -> spin_locked_tree > 0 );
...
static void sync_dkeys( carry_node *node, carry_level *doing )
{
	...
	assert( "nikita-1612", get_lock_counters() -> spin_locked_dk == 0 );
					       ...
					       * new node will ne added */)
{
	...
	new_znode = new_node( brother, brother -> level );
	if( IS_ERR( new_znode ) )
		/*
