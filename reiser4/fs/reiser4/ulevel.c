...
static void print_page (struct page * page)
{
	...
	info ("PAGE: index %lu, count %d, ino %lu%s%s\nbuffers:\n",
...
{
	info( "filldir: %s (%i), %llx, %lx, %i\n",
	      name, namelen, offset, inode, ftype );
	return 0;
}
		 ...
		 reiser4_tree *tree )
{
	...
	if( !strcmp( argv[ 2 ], "mkr4fs" ) ) {
		...
		ret = do_carry( &lowest_level, NULL );
		printf( "result: %i\n", ret );
		...
		print_tree_rec( "tree:dir", tree, REISER4_NODE_CHECK | ~0ul );
...
static int call_create (struct inode * dir, char * name)
{
	reiser4_context *old_context;
	struct dentry dentry;
	int ret;
...
static ssize_t call_write (struct inode * inode, loff_t offset, unsigned count)
{
	reiser4_context *old_context;
	ssize_t result;
	char * buf;
...
int alloc_extent (reiser4_tree *, tween_coord *,
		  reiser4_lock_handle *, void *);

int vs_test( int argc UNUSED_ARG, char **argv UNUSED_ARG, 
	     reiser4_tree *tree )
{
	znode *root;
...
#define NAME_LENGTH 128
	...
	for (i = 0; i < 2; i ++) {
		...
		/*		printf ("AFTER %d-th CREATE\n", i);
		print_tree_rec ("RESULT:", tree, REISER4_NODE_PRINT_HEADER |
	...
	{ /* emulating read */
		...
		extent_set_start (ext + 1, 10ull);
		extent_set_width (ext + 1, 1ull);

		extent_set_start (ext + 2, 0ull);
		extent_set_width (ext + 2, 1ull);

		extent_set_start (ext + 3, 20ull);
		extent_set_width (ext + 3, 1ull);

		data.length = 4 * sizeof (reiser4_extent);
		data.plugin = plugin_by_id (REISER4_ITEM_PLUGIN_ID, EXTENT_ITEM_ID);

		get_inode_sd_key (inode, &key);
		...
		off = 500;
		count = 20480;
		have_read = call_read (inode, off, count);
		should = ((off + count) > inode->i_size) ? (inode->i_size - off) : count;
		if (have_read != should) {
			info ("read %u, should be %u\n", have_read, should);
			return 0;			
		}
		info ("READ OK\n");
		print_pages ();
...
#if 1
	{
		/* lookup a file and write into it */
		char name [NAME_LENGTH];
...
#if 0
		for (i = 0; i < 2; i ++) {
			call_write (inode, (loff_t)i * blocksize * 10,
				    blocksize);
		}
		print_tree_rec ("AFTER WRITE 1", tree, REISER4_NODE_PRINT_HEADER |
				REISER4_NODE_PRINT_KEYS | REISER4_NODE_PRINT_ITEMS);
		/* it should be now [u|1][0|9][u|1] */
#endif
		call_write (inode, (loff_t)0 * blocksize, 513/*15000*/);
		/* .....  [u|196] */
		
		print_tree_rec ("AFTER WRITE 2", tree, REISER4_NODE_PRINT_HEADER |
				REISER4_NODE_PRINT_KEYS | REISER4_NODE_PRINT_ITEMS);

		/*call_truncate (inode, 22ll * reiser4_get_current_sb ()->s_blocksize);
		print_tree_rec ("AFTER TRUNCATE", tree, REISER4_NODE_PRINT_HEADER |
		REISER4_NODE_PRINT_KEYS | REISER4_NODE_PRINT_ITEMS);*/
	}

#endif

	{ /* iterate over a tree and allocate extents */
		tween_coord coord;
		reiser4_lock_handle lh;
		reiser4_key key;
		int result;
...
int real_main( int argc, char **argv )
{
	...
	reiser4_init_oidmap( reiser4_get_oidmap( &super ) );
