					  ...
					  * 0---paste */ )
{
	...
	result = 0;
	plugin = data -> plugin;
	if( plugin -> u.item.b.estimate != NULL )
		/*
		 * ask item plugin how much space is needed to insert this
		 * item
		 */
		result += plugin -> u.item.b.estimate
			( insertion ? NULL : coord, data );
	else
		/*
		 * reasonable default
		 */
		result += data -> length;
	if( insertion ) {
		plugin = node -> node_plugin;
		/*
		 * and add node overhead
		 */
		if( plugin -> u.node.item_overhead != NULL )
			result += plugin -> u.node.item_overhead( node, 0 );
	}
	return result;
}
					       ...
					       * pointer to new child */ )
{
	...
	build_child_ptr_data( child, op -> u.insert.data );
	return result;
}
					  ...
					  * level */ )
{
	...
	orig_node = node = op -> u.insert.coord -> node;
	tracking  = op -> node;
	...
	for( blk_alloc = 0 ; 
		 ...
		 * including insertion coord into the right neighbor.
		 */
		reiser4_dup_coord( &coord_shadow, op -> u.insert.coord );
		node_shadow = op -> node;
		result = carry_shift_data( RIGHT_SIDE, op -> u.insert.coord,
		...
		if( ( not_enough_space > 0 ) && ( node != coord_shadow.node ) ) {
			 ...
			 */
			reiser4_done_coord( op -> u.insert.coord );
			reiser4_dup_coord( op -> u.insert.coord, &coord_shadow );
			node = op -> u.insert.coord -> node;
			op -> node = node_shadow;
			not_enough_space = free_space_shortage( node, op );
		}
							 ...
							 * inserted/pasted */ )
{
	...
	if( op -> u.insert.type == COPT_PASTE_RESTARTED ) {
		...
		op -> u.insert.coord -> node = op -> node -> real_node;
		 ...
		 */
		if( op -> node -> real_node != op -> u.insert.coord -> node ) {
			op -> node = reiser4_add_carry( doing, POOLO_AFTER, 
							op -> node );
			if( IS_ERR( op -> node ) )
				return PTR_ERR( op -> node );
			op -> node -> node = op -> u.insert.coord -> node;
			op -> node -> free = 1;
			...
			carry_level *todo /* next carry level */ )
{
	 ...
	 */
	iplug = coord_of_item( op -> u.insert.coord ) ? 
		&item_plugin_by_coord( op -> u.insert.coord ) -> u.item : NULL;
	can_paste_here = 
		coord_of_item( op -> u.insert.coord ) &&
		( iplug == &op -> u.insert.data -> plugin -> u.item );

	if( !can_paste_here || 
	    ( can_paste_here &&
	      !item_can_contain_key( op -> u.insert.coord, 
				     op -> u.insert.key ) ) ) {
		op -> op = COP_INSERT;
	...
	assert( "nikita-1286", coord_of_item( op -> u.insert.coord ) );
	assert( "nikita-1287", 
		item_plugin_by_coord( op -> u.insert.coord ) -> h.id == 
		op -> u.insert.data -> plugin -> h.id );

	assert( "nikita-992", iplug != NULL );
	real_size = space_needed_for_op( node, op );
	if( real_size > 0 )
		node -> node_plugin ->
			u.node.change_item_size( op -> u.insert.coord, 
						 real_size );
	doing -> restartable = 0;
	result = iplug -> b.paste( op -> u.insert.coord,
					 op -> u.insert.data, todo );
	if( real_size < 0 )
		node -> node_plugin ->
			u.node.change_item_size( op -> u.insert.coord, 
	 ...
	 */
	if( ( op -> u.insert.coord -> unit_pos == 0 ) &&
	    ( op -> u.insert.coord -> between != AFTER_UNIT ) ) {
		reiser4_key item_key;

		unit_key_by_coord( op -> u.insert.coord, &item_key );
		node -> node_plugin -> u.node.update_item_key 
