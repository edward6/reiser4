...
typedef struct lnode_reiser4 {
	lnode_header h;
	struct super_block * l_sb;
	reiser4_inode *p;
} lnode_reiser4;
...
union lnode {
	...
	lnode_inode   l_inode; /*???? for delete ?*/
	lnode_reiser4 l_reiser4;
	lnode_lw      l_lw;     /* ?? */
	lnode_coord   l_coord;  /* ?? */
	lnode_pseudo  l_pseudo; /*??? for delete ?*/
};
...
TYPE_SAFE_HASH_DECLARE(info, reiser4_inode);
