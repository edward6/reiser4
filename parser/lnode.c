...
static int lnodes_init(void);
static int lnodes_done(void);
static lnode * lget( lnode_type type, oid_t oid );
static void lput(lnode * node );
znode *get_vroot(reiser4_tree *tree, reiser4_inode *info);
void set_vroot(reiser4_inode *info, znode * vroot);

extern kmem_cache_t *sys_info_cache;
...
lnodes_init(void)
{
	ln_hash_init(&lnode_htable, LNODE_HTABLE_BUCKETS);
	return 0;
}
...
/* Audited by: green(2002.06.15) */
static lnode *
lget( lnode_type type /* lnode type */ ,
      oid_t oid /* objectid */ )
{
	lnode *result;
...
lput(lnode * node /* lnode to release */ )
{
	...
	if (--node->h.ref == 0) {
		...
		if (node->h.type == LNODE_REISER4_INODE) {
			assert("VD", node->l_reiser4.p != NULL);
			kmem_cache_free(sys_info_cache, node->l_reiser4.p);
		}
		kfree(node);
	}
	spin_unlock(&lnode_guard);
...
znode *l_inode_get_vroot(lnode_reiser4 *l_inode)
{
	return get_vroot(get_tree(l_inode->l_sb), l_inode->p);
}

void l_inode_set_vroot(lnode_reiser4 *l_inode, znode * vroot)
{
	set_vroot(l_inode->p, vroot);
}

reiser4_tree * tree_by_lnode(struct lnode_reiser4 * object)
{
	return get_tree(object->l_sb);
}
