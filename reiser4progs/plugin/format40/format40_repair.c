...
static long int __get_number(int *error, char *ask, ...) {
	...
	*error = 0;
	...
	if (!aal_strncmp(answer, "\n", 1)) {
	    *error = 1;
	    break;
	} else if (!(result = reiserfs_misc_strtol(answer, error)) && *error) {
...
errno_t format40_check(reiserfs_entity_t *entity, uint16_t options) {
    reiserfs_format40_super_t *super;
    reiserfs_format40_t *format = (reiserfs_format40_t *)entity;
...
void format40_print(char *buf, size_t n, reiserfs_entity_t *entity, uint16_t options) 
{
    reiserfs_format40_super_t *super;
    ...
    reiserfs_misc_strcat(buf, n, "Format40 on-disk format\n");
    reiserfs_misc_strcat(buf, n, "Count of blocks free/all: (%llu)/(%llu)\n", 
	get_sb_free_blocks(super), get_sb_block_count(super));
    reiserfs_misc_strcat(buf, n, "Root block (%llu)\n", get_sb_root_block(super));
    reiserfs_misc_strcat(buf, n, "Drop policy (%u)\n", get_sb_drop_policy(super));
    reiserfs_misc_strcat(buf, n, "Oid (%llu)\n", get_sb_oid(super));
    reiserfs_misc_strcat(buf, n, "File count (%llu)\n", get_sb_file_count(super));
    reiserfs_misc_strcat(buf, n, "Flushes (%llu)\n", get_sb_flushes(super));
    reiserfs_misc_strcat(buf, n, "Magic (%s)\n", super->sb_magic);
    reiserfs_misc_strcat(buf, n, "Tree height (%u)\n", get_sb_tree_height(super));
}
