...
static reiserfs_plugin_t *__choose_format(reiserfs_fs_t *fs, aal_device_t *host_device) {
    ...
    if (!(plugin = reiserfs_master_guess(host_device))) {
...
errno_t repair_format_check(reiserfs_fs_t *fs) {
	...
	if (!(fs->format = reiserfs_format_create(repair_data(fs)->host_device, 0, 
    ...
    if (libreiser4_plugin_call(return -1, fs->format->plugin->format_ops, check, 
...
void repair_format_print(FILE *stream, reiserfs_fs_t *fs, uint16_t options) {
    char buf[4096];
