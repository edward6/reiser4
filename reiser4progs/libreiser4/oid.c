...
error_t reiserfs_oid_init(reiserfs_fs_t *fs) {
    ...
    if (!(fs->oid->plugin = libreiser4_plugins_find_by_coords(REISERFS_OID_PLUGIN, plugin_id))) {
...
uint64_t reiserfs_oid_root_parent_objectid(reiserfs_fs_t *fs) {
    ...
    reiserfs_check_method(fs->oid->plugin->oid, root_parent_objectid, return 0);
    return fs->oid->plugin->oid.root_parent_objectid(fs->oid->entity);
}
...
uint64_t reiserfs_oid_root_objectid(reiserfs_fs_t *fs) {
    ...
    reiserfs_check_method(fs->oid->plugin->oid, root_objectid, return 0);
    return fs->oid->plugin->oid.root_objectid(fs->oid->entity);
}
