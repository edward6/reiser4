...
error_t reiserfs_format_open(reiserfs_fs_t *fs) {
    ...
    reiserfs_check_method(plugin->format, open, goto error_free_format);
    if (!(fs->format->entity = plugin->format.open(fs->host_device, 
	fs->journal_device))) 
    {
	aal_exception_throw(EXCEPTION_FATAL, EXCEPTION_OK,
...
void reiserfs_format_close(reiserfs_fs_t *fs) {
    ...
    reiserfs_check_method(fs->format->plugin->format, close, goto error_free_format);
