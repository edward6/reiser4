...
#define REISERFS_ROOT_PARENT_OID 41
#define REISERFS_ROOT_OID 42
...
blk_t hack_create_tree(reiserfs_fs_t *fs, reiserfs_plugin_id_t node_plugin_id) {
    ...
    set_key_locality(&item->key, REISERFS_ROOT_PARENT_OID);
    set_key_objectid(&item->key, REISERFS_ROOT_OID);
    ...
    set_key_locality(&item->key, REISERFS_ROOT_OID);
	...
	set_key_locality(&key, REISERFS_ROOT_OID);
    ...
    */
	...
	set_key_locality(&key, REISERFS_ROOT_PARENT_OID);
	set_key_objectid(&key, REISERFS_ROOT_OID);
	...
	aal_memset(&key, 0, sizeof(key));
	set_key_type(&key, KEY_SD_MINOR);
	set_key_locality(&key, (REISERFS_ROOT_PARENT_OID - 3));
	set_key_objectid(&key, REISERFS_ROOT_PARENT_OID);
