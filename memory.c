 ...
 * "fake" inode (one per super block) and store nodes content into pages bound
 * to this inode in the page cache. In newer kernels bread() already uses
 * inode attached to block device (bd_inode). Advantage of having our own fake
 * inode is that we can install appropriate methods in its address_space
 * operations. Such methods are called by VM on memory pressure (or during
 * background page flushing) and we can use them to react appropriately.
 *
