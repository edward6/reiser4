 ...
 * of a twig in flush_right_relocate_end_of_twig().  As we finish processing a twig we may
 ...
 * 
 * Next we call the main flush routine, flush_squalloc_right, which iterates along the
 * leaf level, squeezing and allocating nodes (and placing them into the flush queue).
 *
 * After flush_squalloc_right returns we take extra steps to ensure that all the children
 * of the final twig node are allocated--this involves repeating flush_squalloc_right
...
int jnode_flush (jnode *node, int *nr_to_flush, int flags)
{
	...
	if (jnode_is_unformatted (left_scan.node)) {
		...
		move_lc (& flush_pos.parent_load, & left_scan.parent_load);
	} else {
 ...
 * reverse parent-first relocate context.  In the forward parent-first relocate context
 * (not here) we actually call the block allocator to try and find a closer location.
 * Here all we know is the preceder and the block number. */
...
/*
 * FIXME: JMACD->HANS: I AM STILL REVIEWING COMMENTS AND COMMENTING ALL BELOW THIS POINT.
 */
...
static int flush_right_relocate_end_of_twig (flush_position *pos)
{
	...
	if ((ret = load_lc_znode (& right_load, right_lock.node))) {
		goto exit;
	}
 ...
 * child that we encounter during flush_allocate_ancestors.  It helps to draw a
 * picture...
 */
...
static int flush_squalloc_changed_ancestors (flush_position *pos)
{
 ...
 repeat:
	...
	if (is_unformatted) {
		done_lc (& pos->parent_load);
		done_lh (& pos->parent_lock);
		move_lh (& pos->parent_lock, & right_lock);
		if ((ret = load_lc_znode (& pos->parent_load, pos->parent_lock.node))) {
				    ...
				    znode_lock_mode parent_mode)
{
	...
	if (jnode_is_unformatted (node)) {
		...
		if ((ret = load_lc_znode (parent_zh, parent_lh->node))) {
			return ret;
		}
		...
		if (coord != NULL) {
			...
			if ((ret = load_lc_znode (parent_zh, parent_lh->node))) {
				warning ("jmacd-976812", "load_lc_znode failed: %d", ret);
				return ret;
			}
...
static void flush_scan_init (flush_scan *scan)
{
	...
	init_lc (& scan->parent_load);
	init_lc (& scan->node_load);
...
static void flush_scan_done (flush_scan *scan)
{
	done_lc (& scan->node_load);
	if (scan->node != NULL) {
		jput (scan->node);
		scan->node = NULL;
	}
	done_lc (& scan->parent_load);
...
static int flush_scan_set_current (flush_scan *scan, jnode *node, unsigned add_count, const coord_t *parent)
{
	...
	/* Failure may happen at the load_lc call, but the caller can assume the reference
	 * is safely taken. */
	return load_lc_jnode (& scan->node_load, node);
}
...
static int flush_scan_common (flush_scan *scan, flush_scan *other)
{
	...
	if (jnode_is_unformatted (scan->node)) {
		...
		if (coord_is_invalid (& scan->parent_coord)) {
			...
			copy_lc (& other->parent_load, & scan->parent_load);
		}
...
static int flush_scan_extent (flush_scan *scan, int skip_first)
{
	...
	for (; ! flush_scan_finished (scan); skip_first = 0) {
		...
		if (coord_is_after_sideof_unit (& next_coord, scan->direction)) {
			...
			if ((ret = load_lc_znode (& next_load, next_lock.node))) {
				goto exit;
			}
		...
		if (next_load.node != NULL) {
			...
			move_lc (& scan->parent_load, & next_load);
		}
...
static int flush_pos_init (flush_position *pos, int *nr_to_flush)
{
	...
	init_lc (& pos->parent_load);
	init_lc (& pos->point_load);
...
static int flush_pos_set_point (flush_position *pos, jnode *node)
{
	...
	pos->point = jref (node);
	return load_lc_jnode (& pos->point_load, node);
}
...
static int flush_pos_lock_parent (flush_position *pos, coord_t *parent_coord, lock_handle *parent_lock, load_count *parent_load, znode_lock_mode mode)
{
	...
	if (flush_pos_unformatted (pos)) {
		...
		if ((ret = load_lc_znode (parent_load, parent_lock->node))) {
			return ret;
		}
...
static const char* flush_pos_tostring (flush_position *pos)
{
	...
	if (pos->parent_lock.node != NULL) {
		...
		if (load_lc_znode (& load, pos->parent_lock.node)) {
			return "*error*";
		}
