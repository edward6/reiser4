...
static int flush_scan_extent_coord (flush_scan *scan, tree_coord *coord)
{
	jnode *neighbor;
	...
	if ((ret = extent_get_inode (coord, & ino))) {
		goto exit;
	}
	...
	for (;; skip_first = 0) {
		...
		if (! item_is_extent (& scan->parent_coord)) {
			if (skip_first) {
				...
				item_plugin *iplug = item_plugin_by_coord (& scan->parent_coord);
				jnode *child;
				
				if ((ret = iplug->common.utmost_child (& scan->parent_coord, sideof_reverse (scan->direction), & child))) {
					return ret;
				}
